------------------------------------------------------------------------
-- Policy Administration System (PAS) - PostgreSQL adaptation
-- Ported from sql/ddl/create-tables.sql (DB2 v12 for z/OS)
--
-- This file is used by the docker-compose local dev environment.
-- DB2-specific syntax (TABLESPACE, STOGROUP, IN ... clauses,
-- CURRENT TIMESTAMP vs CURRENT_TIMESTAMP) has been adjusted for
...
CREATE INDEX IX_POL_STATUS
    ON POLICIES (POLICY_STATUS, EXPIRY_DATE);

CREATE INDEX IX_POL_HOLDER
    ON POLICIES (POLICYHOLDER_ID);

CREATE INDEX IX_POL_AGENT
    ON POLICIES (AGENT_CODE);
...
CREATE INDEX IX_PH_NAME
    ON POLICY_HOLDERS (LAST_NAME, FIRST_NAME);

CREATE INDEX IX_PH_COMPANY
    ON POLICY_HOLDERS (COMPANY_NAME);
...
------------------------------------------------------------------------
CREATE TABLE TERRITORY_FACTORS (
